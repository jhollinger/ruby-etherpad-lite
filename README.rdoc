...
*NOTE* Support for Ruby 1.8.x has been dropped. Upgrade to Ruby 1.9.x.
...
or you can explicitly load just the low-level client:
    ...
    client = EtherpadLite.client('http://beta.etherpad.org/api', 'api key')

The methods available to the low-level client precisely match the HTTP API. In general, it behaves identically to the other clients, particularly PHP's and Python's.
