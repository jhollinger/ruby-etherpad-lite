...
Connecting to Etherpad Lite instances over SSL/HTTPS should "just work". If etherpad-lite can't find your CA Certificates, a warning will be printed. You may tell etherpad-lite where they are with

    EtherpadeLite::Instance.ca_path = '/path/to/ca/certs'

If verifying peers isn't that important to you, you may notice improved speed by setting the above to nil.

== Status

This is alpha or pre-alpha, as only half of the Etherpad Lite is implemented. Notably Authors and Sessions are still missing, but they are in-progress.
